...
const { killPortProcess: killPort } = require("kill-port-process");
...
function start(
  ...
  const startFullNode = new Promise((resolve, reject) => {
...
${data}`);
  ...
  if (killPortProcess) {
    console.log(
      "[conflux-local-network-lite] try kill process on port " + port
    );
    return killPort(port).then(() => startFullNode);
  }

  return startFullNode;
}
