    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        console.log(
          ...
          "\\\n    ",
          "--log-level",
          process.env.CFX_LITE_NODE_ENV === "debug" || verbose
            ? "debug"
            : "error",
          "\n",
          "at",
...
${data}`)
      ...
      Promise.all([
        pretry(_untilFullnodeReady.bind(cfx), {
          ...
          onFailedAttempt: async () => {
            if (verbose)
              console.log(`${LOG_PREFIX} waiting for fullnode to be ready`)
            await delay(100)
          },
        }).then(() => {
        ...
        pretry(_untilFullnodeCaughtUp.bind(cfx), {
          ...
          onFailedAttempt: async () => {
            if (verbose)
              console.log(`${LOG_PREFIX} waiting for fullnode to caught up`)
            await delay(100)
          },
        }).then(() => {
