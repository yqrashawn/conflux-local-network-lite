...
async function sendCFX({ address, privateKey, balance }, web3) {
  ...
  const gas = await web3.estimateGas(txParams).catch(console.error);
  txParams.gas = gas;
  const { transactionHash } = await web3
    .sendTransaction(txParams)
    .confirmed()
    .catch(console.error);
  return await web3.getTransactionReceipt(transactionHash).catch(console.error);
}
...
//   const receipt = await web3.getTransactionReceipt(txHash).catch(console.error);
//   if (receipt === null) {
//     retryCount++;
